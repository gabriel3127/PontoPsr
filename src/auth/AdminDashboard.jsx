...
function AdminDashboard() {
  ...
  // 1. INDIVIDUAL
  useEffect(() => {
    if (view === 'banco' && bancoFilter === 'individual' && bancoFuncionario) {
      setBancoHorasData({})
      loadBancoHoras()
    }
  }, [view, bancoFilter, bancoFuncionario, bancoYear])

  // 2. PERÍODO
  useEffect(() => {
    if (view === 'banco' && bancoFilter === 'periodo' && bancoFuncionario && bancoMesInicio && bancoMesFim) {
      setBancoHorasData({})
      loadBancoHoras()
    }
  }, [view, bancoFilter, bancoFuncionario, bancoYear, bancoMesInicio, bancoMesFim])

  // 3. TODOS
  useEffect(() => {
    if (view === 'banco' && bancoFilter === 'todos') {
      setBancoHorasData({})
      loadAllBancoHoras()
    }
  }, [view, bancoFilter, bancoTodosCategoria, bancoYear])
